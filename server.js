...
function loadDatabase(){
	...
	readYaml('database.yml', function(e, data){
		if (e) throw e;
		if (data){
...
function saveDatabase(){
	...
	yaml('database.yml', database, function(e){
		console.log(e);
	});
}
...
function deleteComment(url, request){
	const id = Number(url.split('/').filter(segment => segment)[1]);
	const savedComment = database.articles[id];
	const response = {};
	...
	if (savedComment){
	    ...
	    const articleCommentIds = database.articles[savedComment.id].commentIds;
	    articleCommentIds.splice(articleCommentIds.indexOf(id), 1);
		response.status = 204;
	}else if (savedComment === undefined){
		response.status = 404;
	}else{
		response.status = 400;
	}
